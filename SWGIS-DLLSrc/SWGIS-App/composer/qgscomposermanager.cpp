...
QgsComposerManager::QgsComposerManager( QWidget * parent, Qt::WindowFlags f ): QDialog( parent, f )
{
  ...
  connect( SWGISApp::instance(), SIGNAL( composerAdded( QgsComposerView* ) ), this, SLOT( refreshComposers() ) );
  connect( SWGISApp::instance(), SIGNAL( composerRemoved( QgsComposerView* ) ), this, SLOT( refreshComposers() ) );
...
void QgsComposerManager::refreshComposers()
{
  ...
  QSet<QgsComposer*> composers = SWGISApp::instance()->printComposers();
...
void QgsComposerManager::on_mAddButton_clicked()
{
  ...
  QString title;
  if ( !SWGISApp::instance()->uniqueComposerTitle( this, title, true ) )
  {
    return;
  }

  newComposer = SWGISApp::instance()->createNewComposer( title );
  if ( !newComposer )
  {
  ...
  if ( loadingTemplate )
  {
    ...
    if ( templateDoc.setContent( &templateFile, false ) )
    {
      ...
      dlg->setStyleSheet( SWGISApp::instance()->styleSheet() );
      dlg->show();
  ...
  {
    newComposer->close();
    SWGISApp::instance()->deleteComposer( newComposer );
    newComposer = nullptr;
...
void QgsComposerManager::showEvent( QShowEvent* event )
{
  ...
  {
    SWGISApp::instance()->addWindow( mWindowAction );
  }
}
...
void QgsComposerManager::remove_clicked()
{
  ...
  {
    SWGISApp::instance()->deleteComposer( c );
  }
}
...
void QgsComposerManager::duplicate_clicked()
{
  ...
  QString newTitle;
  if ( !SWGISApp::instance()->uniqueComposerTitle( this, newTitle, false, currentTitle + tr( " copy" ) ) )
  {
    return;
  }
  ...
  QgsComposer* newComposer = SWGISApp::instance()->duplicateComposer( currentComposer, newTitle );
...
void QgsComposerManager::rename_clicked()
{
  ...
  QString newTitle;
  if ( !SWGISApp::instance()->uniqueComposerTitle( this, newTitle, false, currentTitle ) )
  {
    return;
  }
...
void QgsComposerNameDelegate::setModelData( QWidget *editor, QAbstractItemModel *model, const QModelIndex &index ) const
{
  ...
  QStringList cNames;
  Q_FOREACH ( QgsComposer* c, SWGISApp::instance()->printComposers() )
  {
    cNames << c->title();
  }
