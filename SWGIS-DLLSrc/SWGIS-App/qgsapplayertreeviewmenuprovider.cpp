...
QMenu* QgsAppLayerTreeViewMenuProvider::createContextMenu()
{
  ...
  else if ( QgsLayerTreeNode* node = mView->layerTreeModel()->index2node( idx ) )
  {
    ...
    if ( QgsLayerTree::isGroup( node ) )
    {
      ...
      menu->addAction( QgsApplication::getThemeIcon( "/mActionRemoveLayer.svg" ), tr( "&Remove" ), SWGISApp::instance(), SLOT( removeLayer() ) );

      menu->addAction( QgsApplication::getThemeIcon( "/mActionSetCRS.png" ),
                       tr( "&Set Group CRS" ), SWGISApp::instance(), SLOT( legendGroupSetCRS() ) );

      menu->addAction( actions->actionRenameGroupOrLayer( menu ) );

      menu->addAction( tr( "&Set Group WMS data" ), SWGISApp::instance(), SLOT( legendGroupSetWMSData() ) );
      ...
      if ( SWGISApp::instance()->clipboard()->hasFormat( QGSCLIPBOARD_STYLE_MIME ) )
      {
        menu->addAction( tr( "Paste Style" ), SWGISApp::instance(), SLOT( applyStyleToGroup() ) );
      }

      menu->addAction( tr( "Save As Layer Definition File..." ), SWGISApp::instance(), SLOT( saveAsLayerDefinition() ) );
    ...
    else if ( QgsLayerTree::isLayer( node ) )
    {
      ...
      if ( rlayer )
      {
        menu->addAction( QgsApplication::getThemeIcon( "/mActionZoomActual.svg" ), tr( "&Zoom to Native Resolution (100%)" ), SWGISApp::instance(), SLOT( legendLayerZoomNative() ) );

        if ( rlayer->rasterType() != QgsRasterLayer::Palette )
          menu->addAction( tr( "&Stretch Using Current Extent" ), SWGISApp::instance(), SLOT( legendLayerStretchUsingCurrentExtent() ) );
      }

      menu->addAction( QgsApplication::getThemeIcon( "/mActionRemoveLayer.svg" ), tr( "&Remove" ), SWGISApp::instance(), SLOT( removeLayer() ) );

      // duplicate layer
      QAction* duplicateLayersAction = menu->addAction( QgsApplication::getThemeIcon( "/mActionDuplicateLayer.svg" ), tr( "&Duplicate" ), SWGISApp::instance(), SLOT( duplicateLayers() ) );

      if ( layer && layer->isSpatial() )
      {
        // set layer scale visibility
        menu->addAction( tr( "&Set Layer Scale Visibility" ), SWGISApp::instance(), SLOT( setLayerScaleVisibility() ) );

        if ( !layer->isInScaleRange( mCanvas->scale() ) )
          menu->addAction( tr( "Zoom to &Visible Scale" ), SWGISApp::instance(), SLOT( zoomToLayerScale() ) );

        // set layer crs
        menu->addAction( QgsApplication::getThemeIcon( "/mActionSetCRS.png" ), tr( "&Set Layer CRS" ), SWGISApp::instance(), SLOT( setLayerCRS() ) );

        // assign layer crs to project
        menu->addAction( QgsApplication::getThemeIcon( "/mActionSetProjectCRS.png" ), tr( "Set &Project CRS from Layer" ), SWGISApp::instance(), SLOT( setProjectCRSFromLayer() ) );
      }
      ...
      if ( layer && mView->selectedLayerNodes().count() == 1 )
      {
        ...
        SWGISApp *app = SWGISApp::instance();
      ...
      if ( vlayer )
      {
        QAction *toggleEditingAction = SWGISApp::instance()->actionToggleEditing();
        QAction *saveLayerEditsAction = SWGISApp::instance()->actionSaveActiveLayerEdits();
        QAction *allEditsAction = SWGISApp::instance()->actionAllEdits();

        // attribute table
        menu->addAction( QgsApplication::getThemeIcon( "/mActionOpenTable.svg" ), tr( "&Open Attribute Table" ),
                         SWGISApp::instance(), SLOT( attributeTable() ) );
        ...
        // save as vector file
        menu->addAction( tr( "Save As..." ), SWGISApp::instance(), SLOT( saveAsFile() ) );
        menu->addAction( tr( "Save As Layer Definition File..." ), SWGISApp::instance(), SLOT( saveAsLayerDefinition() ) );

        if ( vlayer->dataProvider()->supportsSubsetString() )
        {
          QAction *action = menu->addAction( tr( "&Filter..." ), SWGISApp::instance(), SLOT( layerSubsetString() ) );
      ...
      {
        menu->addAction( tr( "Save As..." ), SWGISApp::instance(), SLOT( saveAsRasterFile() ) );
        menu->addAction( tr( "Save As Layer Definition File..." ), SWGISApp::instance(), SLOT( saveAsLayerDefinition() ) );
      }
      ...
      if ( layer && QgsProject::instance()->layerIsEmbedded( layer->id() ).isEmpty() )
        menu->addAction( tr( "&Properties" ), SWGISApp::instance(), SLOT( layerProperties() ) );
      ...
      {
        menu->addAction( tr( "Edit virtual layer settings" ), SWGISApp::instance(), SLOT( addVirtualLayer() ) );
      }
    }
    ...
    for ( int i = 0; i < lyrActions.count(); i++ )
    {
      if ( lyrActions[i].allLayers || lyrActions[i].layers.contains( layer ) )
      {
        ...
        else
        {
          ...
          // adapted from SWGISApp::getPluginMenu( QString menuName )
