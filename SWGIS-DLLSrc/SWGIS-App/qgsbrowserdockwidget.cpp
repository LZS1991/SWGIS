...
void QgsBrowserLayerProperties::setItem( QgsDataItem* item )
{
  ...
  if ( defaultProjectionOption == "prompt" )
  {
    QgsCoordinateReferenceSystem defaultCrs =
      SWGISApp::instance()->mapCanvas()->mapSettings().destinationCrs();
...
void QgsBrowserDockWidget::showEvent( QShowEvent * e )
{
  ...
  if ( !mModel )
  {
    ...
    connect( SWGISApp::instance(), SIGNAL( newProject() ), mModel, SLOT( updateProjectHome() ) );
...
void QgsBrowserDockWidget::addLayer( QgsLayerItem *layerItem )
{
  ...
  QString uri = SWGISApp::instance()->crsAndFormatAdjustedLayerUri( layerItem->uri(), layerItem->supportedCRS(), layerItem->supportedFormats() );
  if ( uri.isEmpty() )
    return;
  ...
  QString providerKey = layerItem->providerKey();
    ...
    {
        SWGISApp::instance()->addVectorLayer(uri, layerItem->layerName(), providerKey);
    }
    if(type == QgsMapLayer::RasterLayer)
    {
        SWGISApp::instance()->addRasterLayer( uri, layerItem->layerName(), providerKey );
    }
    if ( type == QgsMapLayer::PluginLayer )
    {
        SWGISApp::instance()->addPluginLayer( uri, layerItem->layerName(), providerKey );
    }
}
...
void QgsBrowserDockWidget::addLayerAtIndex( const QModelIndex& index )
{
    ...
    qDebug() << ( QString("index row is : %1").arg(index.row()));
    qDebug() << ( QString( "rowCount() = %1" ).arg( mModel->rowCount( index ) ) );
  ...
  if ( item && item->type() == QgsDataItem::Project )
  {
    ...
    if ( projectItem )
    {
      ...
      SWGISApp::instance()->openFile( projectItem->path() );
