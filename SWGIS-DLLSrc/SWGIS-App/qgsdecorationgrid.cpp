    ...
    : QgsDecorationItem( parent )
{
  ...
  connect(SWGISApp::instance()->mapCanvas(), SIGNAL(mapUnitsChanged()),
...
void QgsDecorationGrid::render( QPainter * p )
{
  ...
  if ( mGridStyle == QgsDecorationGrid::Line )
  {
    ...
    QgsRenderContext context = QgsRenderContext::fromMapSettings( SWGISApp::instance()->mapCanvas()->mapSettings() );
...
{
  return *SWGISApp::instance()->mapCanvas();
}
...
void QgsDecorationGrid::checkMapUnitsChanged()
{
  ...
  QGis::UnitType mapUnits = SWGISApp::instance()->mapCanvas()->mapSettings().mapUnits();
  if ( mEnabled && ( mMapUnits != mapUnits ) )
  {
    mEnabled = false;
    mMapUnits = QGis::UnknownUnit; // make sure isDirty() returns true
    if ( ! SWGISApp::instance()->mapCanvas()->isFrozen() )
    {
      update();
    }
  }
...
bool QgsDecorationGrid::isDirty()
{
  ...
  if ( mMapUnits == QGis::UnknownUnit ||
       mMapUnits != SWGISApp::instance()->mapCanvas()->mapSettings().mapUnits() ||
...
bool QgsDecorationGrid::getIntervalFromExtent( double* values, bool useXAxis )
{
  ...
  double interval = 0;
  QgsRectangle extent = SWGISApp::instance()->mapCanvas()->extent();
  if ( useXAxis )
...
bool QgsDecorationGrid::getIntervalFromCurrentLayer( double* values )
{
  ...
  QgsMapLayer* layer = SWGISApp::instance()->mapCanvas()->currentLayer();
  if ( ! layer )
  {
