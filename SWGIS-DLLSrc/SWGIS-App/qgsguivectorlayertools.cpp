...
bool QgsGuiVectorLayerTools::startEditing( QgsVectorLayer* layer ) const
{
  ...
  if ( !layer->isEditable() && !layer->readOnly() )
  {
    ...
    {
      SWGISApp::instance()->messageBar()->pushMessage( tr( "Start editing failed" ),
          tr( "Provider cannot be opened for editing" ),
          QgsMessageBar::INFO, SWGISApp::instance()->messageTimeout() );
      return false;
    }
  ...
  if ( layer->isModified() )
  {
                                       ...
                                       buttons ) )
    {
      ...
      case QMessageBox::Discard:
        SWGISApp::instance()->mapCanvas()->freeze( true );
        if ( !layer->rollBack() )
        {
          SWGISApp::instance()->messageBar()->pushMessage( tr( "Error" ),
              tr( "Problems during roll back" ),
              QgsMessageBar::CRITICAL );
          res = false;
        }
        SWGISApp::instance()->mapCanvas()->freeze( false );
...
void QgsGuiVectorLayerTools::commitError( QgsVectorLayer* vlayer ) const
{
  ...
  QgsMessageBarItem *errorMsg = new QgsMessageBarItem(
    ...
    0,
    SWGISApp::instance()->messageBar() );
  SWGISApp::instance()->messageBar()->pushItem( errorMsg );
