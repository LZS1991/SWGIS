    ...
    , mDock( nullptr )
{
  ...
  mDock = new QgsDockWidget( tr( "Identify Results" ), SWGISApp::instance() );
  mDock->setObjectName( "IdentifyResultsDock" );
  mDock->setAllowedAreas( Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea );
  mDock->setWidget( this );
  if ( !SWGISApp::instance()->restoreDockWidget( mDock ) )
    SWGISApp::instance()->addDockWidget( Qt::RightDockWidgetArea, mDock );
  else
    SWGISApp::instance()->panelMenu()->addAction( mDock->toggleViewAction() );
...
{
  connect( this, SIGNAL( activateLayer( QgsMapLayer * ) ), SWGISApp::instance(), SLOT( setActiveLayer( QgsMapLayer * ) ) );
  emit activateLayer( layer( lstResults->currentItem() ) );
  disconnect( this, SIGNAL( activateLayer( QgsMapLayer * ) ), SWGISApp::instance(), SLOT( setActiveLayer( QgsMapLayer * ) ) );
}
...
void QgsIdentifyResultsDialog::layerProperties( QTreeWidgetItem *item )
{
  ...
  SWGISApp::instance()->showLayerProperties( vlayer );
}
