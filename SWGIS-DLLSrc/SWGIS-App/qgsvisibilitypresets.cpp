    ...
    : mMenu( new QMenu )
{
  ...
  mMenu->addAction( SWGISApp::instance()->actionShowAllLayers() );
  mMenu->addAction( SWGISApp::instance()->actionHideAllLayers() );
  mMenu->addAction( SWGISApp::instance()->actionShowSelectedLayers() );
  mMenu->addAction( SWGISApp::instance()->actionHideSelectedLayers() );
  mMenu->addSeparator();
...
{
  QgsLayerTreeModel* model = SWGISApp::instance()->layerTreeView()->layerTreeModel();
...
QStringList QgsVisibilityPresets::orderedPresetVisibleLayers( const QString& name ) const
{
  ...
  QgsLayerTreeMapCanvasBridge* bridge = SWGISApp::instance()->layerTreeCanvasBridge();
