...
void QgsApplication::init( QString customConfigPath )
{
  ...
  // QgsDebugMsg( QString( "prefixPath(): %1" ).arg( prefixPath ) );
    ...
    ABISYM( mBuildOutputPath ) = QDir().currentPath();
    QString binDirName = QDir().currentPath().split( '/', QString::SkipEmptyParts ).last();
    QString str = QDir().currentPath().remove(binDirName);
    ABISYM( mBuildSourcePath ) = QDir().currentPath().remove(binDirName);
