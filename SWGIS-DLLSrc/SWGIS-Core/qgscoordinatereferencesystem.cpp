...
{
  bool result = false;
  QRegExp reCrsId( "^(epsg|poqgs|internal)\\:(\\d+)$", Qt::CaseInsensitive );
...
long QgsCoordinateReferenceSystem::poqgsSrid() const
{
  return mSRID;
}
...
bool QgsCoordinateReferenceSystem::writeXML( QDomNode & theNode, QDomDocument & theDoc ) const
{
  ...
  mySridElement.appendChild( theDoc.createTextNode( QString::number( poqgsSrid() ) ) );
