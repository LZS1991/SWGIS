    ...
    if ( id == layerId )
    {
      ...
      if ( !useAbsolutePathes )
      {
        ...
        else if ( provider == "delimitedtext" )
        {
          ...
          {
            //迁移到Qt5之后添加，LZS2851
            QUrlQuery urlSourceQuery(urlSource);
            QUrl urlDest = QUrl::fromLocalFile( absoluteDs.absoluteFilePath() );
            QUrlQuery urlDestQuery;
            urlDestQuery.setQueryItems( urlSourceQuery.queryItems() );
            urlDest.setQuery(urlDestQuery);

            datasource = QString::fromLatin1( urlDest.toEncoded() );
          }
        }
        else
        {
