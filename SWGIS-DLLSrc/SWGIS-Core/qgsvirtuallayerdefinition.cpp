...
QUrl QgsVirtualLayerDefinition::toUrl() const
{
  ...
  {
    if ( l.isReferenced() )
      urlQuery.addQueryItem( "layer_ref", QString( "%1:%2" ).arg( l.reference(), l.name() ) );
    //未修复，LZS2851
//    else
...
//                                     QString( QUrl::toPercentEncoding( l.source() ) ) ).toUtf8() );
  ...
  if ( !uid().isEmpty() )
    urlQuery.addQueryItem( "uid", uid() );

  if ( geometryWkbType() == QgsWKBTypes::NoGeometry )
    urlQuery.addQueryItem( "nogeometry", "" );
  else if ( !geometryField().isEmpty() )
  {
    if ( hasDefinedGeometry() )
      urlQuery.addQueryItem( "geometry", QString( "%1:%2:%3" ).arg( geometryField() ). arg( geometryWkbType() ).arg( geometrySrid() ).toUtf8() );
    else
      urlQuery.addQueryItem( "geometry", geometryField() );
  }
  ...
  for ( int i = 0; i < fields().count(); i++ )
  {
    ...
    if ( f.type() == QVariant::Int )
      urlQuery.addQueryItem( "field", f.name() + ":int" );
    else if ( f.type() == QVariant::Double )
      urlQuery.addQueryItem( "field", f.name() + ":real" );
    else if ( f.type() == QVariant::String )
      urlQuery.addQueryItem( "field", f.name() + ":text" );
  }
