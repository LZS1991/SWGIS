    ...
    : mURI( uri )
{
  ...
  if ( !mURI.hasParam( QgsWFSConstants::URI_PARAM_URL ) )
  {
    ...
    QString srsname = urlQuery.queryItemValue( "SRSNAME" );
    QString bbox = urlQuery.queryItemValue( "BBOX" );
    QString typeName = urlQuery.queryItemValue( "TYPENAME" );
    QString filter = urlQuery.queryItemValue( "FILTER" );
    mAuth.mUserName = urlQuery.queryItemValue( QgsWFSConstants::URI_PARAM_USERNAME );
    mAuth.mPassword = urlQuery.queryItemValue( QgsWFSConstants::URI_PARAM_PASSWORD );
    mAuth.mAuthCfg = urlQuery.queryItemValue( QgsWFSConstants::URI_PARAM_AUTHCFG );

    // Now remove all stuff that is not the core URL
    urlQuery.removeQueryItem( "SERVICE" );
    urlQuery.removeQueryItem( "VERSION" );
    urlQuery.removeQueryItem( "TYPENAME" );
    urlQuery.removeQueryItem( "REQUEST" );
    urlQuery.removeQueryItem( "BBOX" );
    urlQuery.removeQueryItem( "SRSNAME" );
    urlQuery.removeQueryItem( "FILTER" );
    urlQuery.removeQueryItem( QgsWFSConstants::URI_PARAM_USERNAME );
    urlQuery.removeQueryItem( QgsWFSConstants::URI_PARAM_PASSWORD );
    urlQuery.removeQueryItem( QgsWFSConstants::URI_PARAM_AUTHCFG );
...
QUrl QgsWFSDataSourceURI::baseURL( bool bIncludeServiceWFS ) const
{
  ...
  {
    urlQuery.addQueryItem( "SERVICE", "WFS" );
  }
  url.setQuery(urlQuery);
  return url;
}
