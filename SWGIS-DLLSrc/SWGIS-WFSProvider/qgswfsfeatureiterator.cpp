...
QUrl QgsWFSFeatureDownloader::buildURL( int startIndex, int maxFeatures, bool forHits )
{
  ...
  QUrlQuery urlQuery(getFeatureUrl);
  urlQuery.addQueryItem( "REQUEST", "GetFeature" );
  urlQuery.addQueryItem( "VERSION",  mShared->mWFSVersion );
  ...
  if ( mShared->mWFSVersion.startsWith( "2.0" ) )
    urlQuery.addQueryItem( "TYPENAMES",  typenames );
  else
    urlQuery.addQueryItem( "TYPENAME",  typenames );
  if ( forHits )
  {
    urlQuery.addQueryItem( "RESULTTYPE", "hits" );
  }
  else if ( maxFeatures > 0 )
  {
    if ( mSupportsPaging )
    {
      ...
      // at index 254.
      urlQuery.addQueryItem( "STARTINDEX", QString::number( startIndex ) );
    }
    if ( mShared->mWFSVersion.startsWith( "2.0" ) )
      urlQuery.addQueryItem( "COUNT", QString::number( maxFeatures ) );
    else
      urlQuery.addQueryItem( "MAXFEATURES", QString::number( maxFeatures ) );
  }
  QString srsName( mShared->srsName() );
  if ( !srsName.isEmpty() && !forHits )
  {
    urlQuery.addQueryItem( "SRSNAME", srsName );
  }
  ...
  if ( !mShared->mRect.isNull() && !mShared->mWFSFilter.isEmpty() )
  {
    ...
    urlQuery.addQueryItem( "FILTER", sanitizeFilter( doc.toString() ) );
  }
  ...
  {
    urlQuery.addQueryItem( "FILTER", sanitizeFilter( mShared->mWFSFilter ) );
  }

  if ( !mShared->mSortBy.isEmpty() && !forHits )
  {
    urlQuery.addQueryItem( "SORTBY", mShared->mSortBy );
  }

  getFeatureUrl.setQuery(urlQuery);
...
void QgsWFSFeatureDownloader::run( bool serializeFeatures, int maxFeatures )
{
  ...
  while ( true )
  {
    ...
    {
      urlQuery.addQueryItem( "RETRY", QString::number( retryIter ) );
    }
