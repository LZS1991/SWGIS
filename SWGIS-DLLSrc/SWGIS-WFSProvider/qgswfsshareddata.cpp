    ...
    const QString& filter )
{
  ...
  if ( WFSVersion.startsWith( "2.0" ) )
    urlQuery.addQueryItem( "TYPENAMES", mUri.typeName() );
  else
    urlQuery.addQueryItem( "TYPENAME", mUri.typeName() );
  if ( !filter.isEmpty() )
  {
    urlQuery.addQueryItem( "FILTER", filter );
  }
...
QgsRectangle QgsWFSSingleFeatureRequest::getExtent()
{
  ...
  if ( mShared->mWFSVersion .startsWith( "2.0" ) )
    ...
    urlQuery.addQueryItem( "TYPENAME", mUri.typeName() );
  if ( mShared->mWFSVersion .startsWith( "2.0" ) )
    urlQuery.addQueryItem( "COUNT", QString::number( 1 ) );
  else
    urlQuery.addQueryItem( "MAXFEATURES", QString::number( 1 ) );
  getFeatureUrl.setQuery(urlQuery);
  if ( !sendGET( getFeatureUrl, true ) )
    return -1;
