...
void QgsWmsProvider::setQueryItem( QUrl &url, const QString& item, const QString& value )
{
    ...
    urlQuery.removeQueryItem(item);
    urlQuery.addQueryItem(item, value);
    ...
    switch ( tileMode )
    {
      ...
      case WMTS:
      {
        if ( !getTileUrl().isNull() )
        {
          ...
          QUrlQuery urlQuery(url);
          urlQuery.removeQueryItem( "TILEROW" );
          urlQuery.removeQueryItem( "TILECOL" );
          url.setQuery(urlQuery);
          int i = 0;
          for ( int row = row0; row <= row1; row++ )
          {
